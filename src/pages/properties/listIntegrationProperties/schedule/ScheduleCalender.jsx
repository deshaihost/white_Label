...
const ScheduleCalender = ({
  ...
  const [showCopyToPropertiesModal, setShowCopyToPropertiesModal] =
    useState(false);
  ...
  const handleRemoveSchedule = (deleteData) => {
    ...
    if (
      !deleteData.scheduleStage ||
      !deleteData.day ||
      !deleteData.startTime ||
      !deleteData.endTime ||
      deleteData.startIndex === undefined ||
      deleteData.endIndex === undefined
    ) {
      ToastHandle("Something went wrong here", "danger");
      return;
    }
    responseObject.schedules[deleteData.scheduleStage][deleteData.day].splice(
      deleteData.startIndex,
      2
    );
  ...
  return (
    <>
        ...
        <>
            ...
            <div className="container">
              <div className="row">
                <div className="col-md-8">
                  {daysOfWeek?.map((weeks) => {
                    let findRang = scheduleDefulatArray?.map(
                      (schedule, index) => {
                        let daysTime = scheduledDate?.[schedule][weeks];
                        let startTime;
                        let startIndex;
                        let endTime;
                        let endIndex;
                  ...
                  <div className="px-0 px-lg-3 px-md-3 px-xxl-5">
                    <div className="border border-primary border-2 p-4 rounded-3">
                      ...
                      <p className="fs-14 mb-3">
                        Colored dots indicate that HostBuddy will respond to
                        guests at reservation stages:
                      </p>
                      <p className="fs-14">
          ...
          <div class="row w-full mt-3 mb-3 d-flex justify-content-center">
            <div className="d-flex gap-3 flex-wrap flex-md-nowrap" style={{ width: "80%" }}>
              ...
              <button
                onClick={() => setShowCopyToPropertiesModal(true)}
                className="btn btn-primary form-control"
              >
      ...
      <CopyToPropertiesModal
        show={showCopyToPropertiesModal}
        setShow={setShowCopyToPropertiesModal}
