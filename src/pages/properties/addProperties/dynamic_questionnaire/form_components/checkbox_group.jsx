...
const CheckboxGroupComponent = ({ question_object, sec_name, subsec_name, q_ind, handleInputComponentChange, handlePencilIconClick }) => {
  ...
  // When the component mounts, use setCheckboxStatuses to populate previously checked boxes
  useEffect(() => {
            ...
            return (
              <li className="amenties-list-item">
                <div className={ checkboxStatuses[option_index] ? "form-checkbox bg-light text-dark" : "form-checkbox" }>
                  ...
                  {checkboxStatuses[option_index] && <PencilIcon sec_name={sec_name} subsec_name={subsec_name} q_ind={q_ind} checkbox_group_option={option} handlePencilIconClick={handlePencilIconClick} someResStageIsSelected={someResStageIsSelected}/>}
                </div>
              </li>
            );
          })}
