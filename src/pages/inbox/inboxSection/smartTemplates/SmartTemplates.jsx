...
import SmartTemplateIndex from "./smartTemplateFunctionality/SmartTemplateIndex";
...
const SmartTemplates = ({ allPropertyNamesList }) => {
  ...
  const [getReviewSettingsLoading, setGetReviewSettingsLoading] =
    useState(false);
  ...
  const [getCheckInSettingsLoading, setGetCheckInSettingsLoading] =
    useState(false);
  ...
  const [
    ...
    setGetReviewUpcomingMessagesLoading,
  ] = useState(false);
  const [checkInUpcomingMessagesData, setCheckInUpcomingMessagesData] =
    useState([]);
  const [
    getCheckInUpcomingMessagesLoading,
    setGetCheckInUpcomingMessagesLoading,
  ] = useState(false);
  const [
    propertyReadyUpcomingMessagesData,
    setPropertyReadyUpcomingMessagesData,
  ] = useState([]);
  const [
    getPropertyReadyUpcomingMessagesLoading,
    setGetPropertyReadyUpcomingMessagesLoading,
  ] = useState(false);
  ...
  const callGetSettingsApi = async (upsell_type) => {
    ...
    if (upsell_type === "review_upsell") {
      setGetReviewSettingsLoading(true);
    } else if (upsell_type === "check_in") {
      setGetCheckInSettingsLoading(true);
    } else if (upsell_type === "property_ready") {
      setGetPropertyReadyLoading(true);
    }
    ...
    try {
      ...
      const response = await axios.get(
        `${baseUrl}/get_upsell_settings?upsell_type=${upsell_type}`,
        config
      );
  ...
  const callGetUpcomingMessagesApi = async (regenerate, upsell_type) => {
    ...
    const API_KEY = process.env.REACT_APP_API_KEY;
    if (upsell_type === "review_upsell") {
      setGetReviewUpcomingMessagesLoading(true);
    } else if (upsell_type === "check_in") {
      setGetCheckInUpcomingMessagesLoading(true);
    } else if (upsell_type === "property_ready") {
      setGetPropertyReadyUpcomingMessagesLoading(true);
    }
    ...
    try {
      ...
      const response = await axios.get(
        `${baseUrl}/get_upcoming_messages?upsell_type=${upsell_type}&regenerate=${regenerate}`,
        config
      );

      if (response.status === 200) {
        if (upsell_type === "review_upsell") {
          setReviewUpcomingMessagesData(response?.data?.upcoming_messages);
        } else if (upsell_type === "check_in") {
          setCheckInUpcomingMessagesData(response?.data?.upcoming_messages);
        } else if (upsell_type === "property_ready") {
          setPropertyReadyUpcomingMessagesData(
            response?.data?.upcoming_messages
          );
        }
      } else {
      ...
      if (upsell_type === "review_upsell") {
        setGetReviewUpcomingMessagesLoading(false);
      } else if (upsell_type === "check_in") {
        setGetCheckInUpcomingMessagesLoading(false);
      } else if (upsell_type === "property_ready") {
        setGetPropertyReadyUpcomingMessagesLoading(false);
      }
    }
  };
    ...
    >
      {selectedSection === "reviewUpsells" && (
        <ReviewUpsells
          ...
          settingsApiData={reviewSettingsApiData}
          setSettingsApiData={setReviewSettingsApiData}
          localSettingsData={reviewLocalSettingsData}
          setLocalSettingsData={setReviewLocalSettingsData}
          callGetSettingsApi={callGetSettingsApi}
          getSettingsLoading={getReviewSettingsLoading}
          callGetUpcomingMessagesApi={callGetUpcomingMessagesApi}
          getUpcomingMessagesLoading={getReviewUpcomingMessagesLoading}
      ...
      */}
      ...
      {selectedSection === "propertyReady" && (
        ...
        // userHasTurnoAccount={userHasTurnoAccount}
        />
      )}
      ...
      {selectedSection === "index" && (
        <div className="upsells-settings">
          ...
          <div
            ...
            onClick={() => setSelectedSection("reviewUpsells")}
          >
