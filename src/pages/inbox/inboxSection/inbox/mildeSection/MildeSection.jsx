...
const MildeSection = ({ allConversationData, updateConversationFromApi, updateConversationLocal, subscriptionPlan, accountAgeDays, setCurrentView }) => {
  ...
  const callGenerateFromCommandApi = async (command) => {
    ...
    try {
      ...
      const body_data = { property_name:propertyName, conversation_id, command };
  ...
  const handleSendOptionSelect = (option) => {
    ...
    if (option === 'custom') {
      ...
      // Here you would handle the actual scheduling logic
      ToastHandle(`Message scheduled for the day after tomorrow at 10:30 AM`, "success");
    }
  };
  ...
  const handleClickOutside = (event) => {
    ...
    if (sendMenuRef.current && !sendMenuRef.current.contains(event.target) && sendButtonRef.current && !sendButtonRef.current.contains(event.target)) {
      setSendOptionsVisible(false);
    }
  };
  ...
  const generateCalendarDays = () => {
    ...
    const daysInCurrentMonth = Array.from({ length: daysInMonth }, (_, i) => i + 1);
  ...
  const handleDateSelection = (day, isCurrentMonth = true, event) => {
    ...
    if (!isCurrentMonth) {
      if (day > 20) { // Likely previous month
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      const datePickerElements = document.querySelectorAll('.date-picker-dropdown');
      if (datePickerElements.length && !datePickerElements[0].contains(event.target) && 
  ...
  useEffect(() => {
    ...
    if (allConversationData?.messages) {
      ...
      if (allConversationData?.conversation_id === currentConversationIdRef.current) {
    ...
    } else if (allConversationData?.conversation_id === currentConversationIdRef.current) {
              ...
              <div className="input-container" style={{ width: '100%', marginBottom: '10px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <div className="generate-container">
                    <button 
                      ref={buttonRef} 
                      className={`generate-button ${generateOptionsVisible ? 'active' : ''}`}
                    ...
                    >
                      ...
                      <div ref={menuRef} className="generate-menu" style={{ zIndex: 1000 , border: '1px solid #38383d'}}>
                ...
                <div className="send-container" style={{ position: 'relative' }}>
                <button 
                  onClick={handleSendMessage} 
                  className='chat-send-button' 
                  disabled={generateCommandApiLoading || generateScratchApiLoading || sendMessageLoading || !inputValue.trim()}
                  style={{                                                                                                                                                       
                    backgroundColor: (!inputValue.trim() && !(generateCommandApiLoading || generateScratchApiLoading || sendMessageLoading)) ? 'rgba(15, 17, 23, 0.42)' : '#007bff',
                    color: (!inputValue.trim() && !(generateCommandApiLoading || generateScratchApiLoading || sendMessageLoading)) ? '#4A4D54' : 'white'
                  }}
                >
                  {(sendMessageLoading) ? (
                ...
                </button>                {sendOptionsVisible && (
                  <div ref={sendMenuRef} className="send-menu" style={{ zIndex: 1000 ,border: '1px solid #38383d'}}>
                    <button className="send-menu-item" key='schedule' onClick={() => handleSendOptionSelect('schedule')}>Schedule Message</button>
                    <button className="send-menu-item" key='tomorrow' onClick={() => handleSendOptionSelect('tomorrow')}>Tomorrow at 10:30 AM</button>
                    <button className="send-menu-item" key='nextday' onClick={() => handleSendOptionSelect('nextday')}>Next Day at 10:30 AM</button>
                    <button className="send-menu-item" key='custom' onClick={() => handleSendOptionSelect('custom')}>Customize Time</button>
                  </div>
                )}
                </div>
              </div>
      ...
      {scheduleMessageModalOpen && (
        ...
        >
          ...
          >
            ...
            {/* Modal Body */}
            <div style={{ padding: '0 24px' }}>              {/* Date and Time Selection Section */}
              <div style={{ 
                display: 'flex', 
                gap: '16px', 
                marginBottom: '20px' 
              }}>
                {/* Date Field */}                <div style={{ 
                    flex: 1.5,
                  ...
                  >                    <div
                    ...
                    >
                      ...
                      >
                        <path d="M5.33333 1.33334V3.33334" stroke="white" strokeWidth="1.2" strokeLinecap="round"/>
                        <path d="M10.6667 1.33334V3.33334" stroke="white" strokeWidth="1.2" strokeLinecap="round"/>
                        <path d="M2.33333 6.00001H13.6667" stroke="white" strokeWidth="1.2" strokeLinecap="round"/>
                        <rect x="2.33333" y="3.33334" width="11.3333" height="10.6667" rx="2" stroke="white" strokeWidth="1.2"/>
                      </svg>                        <span style={{ 
                          flex: 1,
                          whiteSpace: 'nowrap',
                          overflow: 'hidden',
                      ...
                      >
                        ...
                        />
                      </svg>                      {/* Calendar Dropdown */}
                      {showDatePicker && (
                        <div 
                          className="date-picker-dropdown"                          style={{
                            position: 'fixed', // Changed to fixed positioning to ensure visibility
                            left: dateFieldRef.current ? dateFieldRef.current.getBoundingClientRect().left : 0,
                            // Intelligently position the calendar either above or below based on available space
                            top: calendarPosition === 'top' 
                              ? (dateFieldRef.current ? dateFieldRef.current.getBoundingClientRect().top - 360 : 'auto')  
                              : (dateFieldRef.current ? dateFieldRef.current.getBoundingClientRect().bottom + 4 : 'auto'),
                            backgroundColor: '#2B2E36',
                            borderRadius: '8px',
                        ...
                        >
                          ...
                          className="date-picker-dropdown">                            {/* Previous month days */}
                            {generateCalendarDays().daysFromPrevMonth.map((day) => {
                              ...
                              return (
                                <div key={`prev-${day}`} style={{ 
                                  ...
                                  borderRadius: '4px',
                                  cursor: 'pointer',
                                  backgroundColor: isSelected ? '#01255E' : 'transparent',
                            ...
                            {/* Current month days */}
                            {generateCalendarDays().daysInCurrentMonth.map((day) => {
                            ...
                            {/* Next month days */}
                            {generateCalendarDays().daysFromNextMonth.map((day) => {
            ...
            >
              ...
              >
                ...
                style={{
                  ...
                  ToastHandle(`Message scheduled for ${getFormattedDate(scheduledDate)} at ${scheduledTime}`, "success");
                }}
              >
                Schedule message
              </button>
