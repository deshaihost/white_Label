...
interface PreStayGapNightProps {
  onBack: () => void;
}

export default function PreStayGapNight({ onBack }: PreStayGapNightProps) {
  ...
  const [message, setMessage] = useState("Hi {{guest_name}}, we'd love for you to arrive earlier! There are {{number_of_nights_available}} night(s) available right before your reservation. If you're interested, we can offer you a discount of {{discount_percentage}} on the gap night. Let us know if this works for you!");
  ...
  const handleSave = () => {
    showSaveSuccess('Pre-stay gap night settings saved successfully');
  };
      ...
      <div className="max-w-[1200px] mx-auto px-12 py-12">
        ...
        <div className="flex items-start justify-between mb-10">
          <div className="flex-1">
            ...
            <p className="text-[#a6a9b2] text-[16px] font-['DM_Sans:Regular',_sans-serif] leading-relaxed max-w-[900px]" style={{ fontVariationSettings: "'opsz' 14" }}>
              HostBuddy will detect when there's a vacant night between two reservations. You'll send a message to the second guest with a gap night upsell offer, customizable by you, with AI personalization if you enable it. If they're interested, HostBuddy will automatically acknowledge and prompt you to manually extend their stay.
            </p>
          </div>
        ...
        <div className="mb-10">
          ...
          <p className={`text-[14px] font-['DM_Sans:Regular',_sans-serif] ${enabled ? 'text-[#4ade80]' : 'text-[#ef4444]'}`} style={{ fontVariationSettings: "'opsz' 14" }}>
            You currently have pre-stay upsells {enabled ? 'enabled' : 'not enabled'}.
          </p>
        </div>
              ...
              <tbody>
                <tr>
                  <td colSpan={5} className="px-6 py-8 text-center text-[#a6a9b2] text-[14px] font-['DM_Sans:Regular',_sans-serif]" style={{ fontVariationSettings: "'opsz' 14" }}>
                    {!enabled ? 'Pre-stay upsells are off' : 'No upcoming messages'}
                  </td>
                </tr>
              </tbody>
            </table>
